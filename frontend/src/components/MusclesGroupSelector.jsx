...
const MusclesGroupSelector = ({ initialSelectedMuscles = [] }) => {
    const [selectedMuscles, setSelectedMuscles] = useState(initialSelectedMuscles);

    const handleAddMuscle = (id) => setSelectedMuscles((prevSelectedMuscles) => { if (!prevSelectedMuscles.includes(id)) return [...prevSelectedMuscles, id]; else return prevSelectedMuscles; })
    const handleRemoveMuscle = (id) => setSelectedMuscles((prevSelectedMuscles) => prevSelectedMuscles.filter(m => m !== id))

    const handleMuscleChange = (id) => {
        const newSelectedMuscles = selectedMuscles.includes(id)
            ? selectedMuscles.filter(muscle => muscle !== id)
            : [...selectedMuscles, id];

        setSelectedMuscles(newSelectedMuscles);
        console.log(newSelectedMuscles)
    };

    useEffect(() => {
      const event = new CustomEvent('musclesSelectionChange', { detail: selectedMuscles });
    ...
    useEffect(() => {
        ...
        const handleClick = (event) => {
            console.log("click")
            let id = event.target.closest('g[id]').id.toLowerCase();
            //handleMuscleChange(id);
            let input = document.getElementById(id);
            if (input) {
                input.checked = !input.checked;
                input.checked ? handleAddMuscle(id) : handleRemoveMuscle(id);
